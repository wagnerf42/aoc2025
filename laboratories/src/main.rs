...
fn question1() -> std::io::Result<()> {
    ...
    let mut splits = 0;
    let end = splitters.fold(std::iter::once(start).collect::<HashSet<_>>(), |mut h, pos| {
        h.iter().flat_map(|&p| {
            if pos.contains(&p) {
                splits += 1;
                [Some(p-1), Some(p+1)]
...
fn question2() -> std::io::Result<()> {
    let mut splitters = parse_splitters("input")?;
    let start = splitters.next().unwrap().into_iter().next().unwrap();
    let end_state = splitters.fold(std::iter::once((start, 1)).collect::<HashMap<_, _>>(), |mut h, pos| {
